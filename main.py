        ...
        for line in map(list,zip(fastq[0],fastq[num])):
            ...
            if len(read) == size:
                ...
                if len(completetrim[0]) != len(initial_read):
                    trimmed_reads += 1
                if meanquality(completetrim[0],quality_coversion[0][:len(completetrim[0])],qualitythreshold=30) or checklen(completetrim[0], minlen=50) is None:
                    print("stuff")
                    removed_reads += 1
                else:
